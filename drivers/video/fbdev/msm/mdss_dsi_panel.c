							...
							int mode)
{
	...
	if (mipi->dms_mode == DYNAMIC_MODE_SWITCH_IMMEDIATE) {
		flags |= (CMD_REQ_DMA_TPG | CMD_REQ_COMMIT);
...
static int mdss_dsi_panel_refresh_rate_ctl(struct mdss_panel_data *pdata, int rate)
{
	...
	if (rate > 60) {
		...
		// Divide frame refresh by 3 using interval driving mode
		non_refresh_frames = 2;
		vfp = ((522547200 / rate / 4) - 4327680) / 1680;
	} else {
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->night_mode_cfg.enabled && ctrl->night_mode_cfg.current_profile >= 0)
		mdss_dsi_panel_night_mode_ctl(pdata, ctrl->night_mode_cfg.current_profile);

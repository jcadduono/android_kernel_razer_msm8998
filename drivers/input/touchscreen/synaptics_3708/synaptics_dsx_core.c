    ...
    if (evdata && evdata->data && rmi4_data)
    {
        if (event == FB_EVENT_BLANK)
        {
...
#ifdef FB_RESUME_WORK_QUEUE
            if (*transition == FB_BLANK_POWERDOWN || *transition == FB_BLANK_UNBLANK)
            {
            ...
            else if (*transition == FB_BLANK_UNBLANK)
            {
                ...
                // alex.naidis@paranoidandroid.co Protect powerup process - start
                if (rmi4_data->init_powerup)
                {
                    rmi4_data->init_powerup = false;
                    return 0;
                }
                // alex.naidis@paranoidandroid.co Protect powerup process - end
#ifdef FB_RESUME_WORK_QUEUE
                queue_delayed_work(rmi4_data->pm_workqueue,
                                   &rmi4_data->pm_work,

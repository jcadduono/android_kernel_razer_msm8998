...
int ALT_readChipInfo()
{
    ...
    if (readCmdU16(FTS_CMD_FRAMEBUFFER_R, ADDR_FRAMEBUFFER_DATA, data, CHIP_INFO_SIZE+3, DUMMY_FRAMEBUFFER) < 0)
    {
        logError(1, "%s ALT_readChipInfo: ERROR %02X\n", tag, ERROR_I2C_R);
        ret = ERROR_I2C_R;
        goto FAIL;
    }
    ...
    if (data[0] != HEADER_SIGNATURE)
    {
        logError(1, "%s ALT_readChipInfo: ERROR %02X The Header Signature was wrong!  %02X != %02X \n", tag, ERROR_WRONG_COMP_SIGN, data[0], HEADER_SIGNATURE);
        ret = ERROR_WRONG_COMP_SIGN;
        goto FAIL;
    }
    ...
    index += 3;
    ALT_ftsInfo.u8_loadCnt = data[index++];
    ALT_ftsInfo.u8_infoVer = data[index++];
    u8ToU16(&data[index], &ALT_ftsInfo.u16_ftsdId);
    index += 2;
    ALT_ftsInfo.u8_ftsdVer = data[index++];
    ALT_ftsInfo.u8_ftsaId = data[index++];
    ALT_ftsInfo.u8_ftsaVer = data[index++];
    ALT_ftsInfo.u8_tchRptVer = data[index++];

    //logError(1, "%s External Release =  ", tag);
    for (i = 0; i < EXTERNAL_RELEASE_INFO_SIZE; i++)
    {
        ALT_ftsInfo.u8_extReleaseInfo[i] = data[index++];
        //logError(1, "%02X ", ALT_ftsInfo.u8_extReleaseInfo[i]);
    }
    //logError(1, "\n");

    for (i = 0; i < sizeof(ALT_ftsInfo.u8_custInfo); i++)
    {
        ALT_ftsInfo.u8_custInfo[i] = data[index++];
    }

    u8ToU16(&data[index], &ALT_ftsInfo.u16_fwVer);
    index += 2;
    //logError(1, "%s FW VERSION = %04X \n", tag, ALT_ftsInfo.u16_fwVer);

    u8ToU16(&data[index], &ALT_ftsInfo.u16_cfgId);
    index += 2;
    //logError(1, "%s CONFIG ID = %04X \n", tag, ALT_ftsInfo.u16_cfgId);

    ftsInfo.u16_fwVer = ALT_ftsInfo.u16_fwVer;
    ftsInfo.u16_cfgId = ALT_ftsInfo.u16_cfgId;
    //logError(1, "%s Chip Info Read DONE! FW VERSION = ST-%04X.%04X\n", tag, ftsInfo.u16_fwVer, ftsInfo.u16_cfgId);

...
#include "../../../fih/fih_touch.h" //SW8-PRODUCTION-JH-Porting touch+
...
//SW8-PRODUCTION-JH-Porting touch+[
extern struct fih_touch_cb touch_cb;
extern int tp_probe_success;
void touch_tpfwver_read(char *fw_ver);
void touch_vendor_read(char *vendor);
void stm_fts_fih_cmd(void);
int stm_fts_fih_read_result(void);
...
/***************************************** FW UPGGRADE ***************************************************/
/* update firmware*/
//echo 01/00 > fwupdate     perform a fw update taking the FW to burn always from a bin file stored in /system/etc/firmware, 01= force the FW update whicheve fw_version and config_id; 00=perform a fw update only if the fw in the file has a greater fw_version or config_id
//cat fwupdate		    to show the result of the burning procedure (example output in the terminal = "AA00000001BB" if the switch is enabled)
//echo 01/00 > fwupdate; cat fwupdate 		to perform both operation stated before in just one call
                                  ...
                                  const char *buf, size_t count)
{
    ...
    if (ret < OK)
    {
        logError(1, "%s fts_fwupdate_store: impossible retrieve FW... ERROR %08X\n", tag, ERROR_MEMH_READ);
    ...
    if (ret < OK)
    {
        logError(1, "%s fts_fwupdate_store: impossible parse ERROR %08X\n", tag, ERROR_MEMH_READ);
        ret = (ret | ERROR_MEMH_READ);
        goto END;
    }
...
//cat mode_active		    to show the bitmask of which indicate the modes/features which are running on the IC in a specific istant oftime (example output in the terminal = "AA10000000BB" only senseOn performed)
...
//cat fw_file_test			show on the terminal fw_version and config_id of the FW stored in the fw file/header file
...
//cat lockdown_info		    to show the lockdown info on the terminal (example output in the terminal = "AA00000000X1X2..X10BB" ) where first 4 bytes correspond t
...
//Example based on the status of the screen and on the feature that is trying to enable
	...
	switch(feature){
		...
		case FEAT_GESTURE:
    #endif
		case FEAT_GLOVE:					//glove mode can only activate during sense on
			res = OK;
		break;
...
//Example based only on the feature that is going to be activated
    ...
    case FEAT_GESTURE:
        if(info->cover_enabled==1)
        {
            ...
            //for example here can be place a code for disabling the cover mode when gesture is activated
        }
        break;
    ...
    case FEAT_COVER:
        if(info->gesture_enabled==1)
        {
            ...
            //for example here can be place a code for disabling the gesture mode when cover is activated (that means that cover mode has an higher priority on gesture mode)
        }
        break;
...
#ifdef USE_ONE_FILE_NODE
//echo XXXX 00/01 > feature_enable     set the feature to disable/enable. XXXX = 4 bytes which identify the feature
//cat feature_enable		   set the enabled mode/features in the IC and return an error code
//echo XXXX 00/01 > feature_enable; cat feature_enable 		to perform both action stated before in just one call
    ...
    else
    {
        ...
        if(res>=OK)
        {
            ...
            case FEAT_EDGE_REJECTION:
                sscanf(p, "%02X ", &info->edge_rej_enabled);
                logError(1, "%s fts_feature_enable: Edge Rejection Enabled = %d \n", tag, info->edge_rej_enabled);
            ...
            case FEAT_CORNER_REJECTION:
                sscanf(p, "%02X ", &info->corner_rej_enabled);
                logError(1, "%s fts_feature_enable: Corner Rejection Enabled = %d \n", tag, info->corner_rej_enabled);
            ...
            case FEAT_EDGE_PALM_REJECTION:
                sscanf(p, "%02X ", &info->edge_palm_rej_enabled);
                logError(1, "%s fts_feature_enable: Edge Palm Rejection Enabled = %d \n", tag, info->edge_palm_rej_enabled);
        ...
        {
            logError(1,"%s %s: Call echo XXXX 00/01 > feature_enable with a correct feature! ERROR %08X \n",tag, __func__,res);
        }
...
static ssize_t fts_feature_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if(feature_feasibility<OK)
    {
        logError(1,"%s %s: Call before echo XXXX 00/01 > feature_enable with a correct feature! ERROR %08X \n",tag, __func__,feature_feasibility);
    }
...
//cat edge_rej		    to show the status of the edge_rej_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
//echo 01/00 > edge_rej; cat edge_rej 		to enable/disable edge rejection and see the switch status in just one call
static ssize_t fts_edge_rej_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char buff[CMD_STR_LEN] = {0};
    int size = 6 * 2;
    u8 *all_strbuff = NULL;
...
static ssize_t fts_edge_rej_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char *p = (char *)buf;
    unsigned int temp;
    int res;
...
//in case of a different elaboration of the input, just modify this initial part of the code
    if ((count + 1) / 3 !=1)
    {
...
//cat corner_rej		    to show the status of the corner_rej_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
//echo 01/00 > corner_rej; cat corner_rej 		to enable/disable corner rejection and see the switch status in just one call
static ssize_t fts_corner_rej_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char buff[CMD_STR_LEN] = {0};
    int size = 6 * 2;
    u8 *all_strbuff = NULL;
...
static ssize_t fts_corner_rej_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char *p = (char *)buf;
    unsigned int temp;
    int res;
...
#ifdef EDGE_PALM_REJ
//echo 01/00 > edge_palm_rej     to enable/disable edge palm rejection
//cat edge_palm_rej		    to show the status of the edge_palm_rej_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
//echo 01/00 > edge_palm_rej; cat edge_palm_rej 		to enable/disable edge palm rejection and see the switch status in just one call
static ssize_t fts_edge_palm_rej_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char buff[CMD_STR_LEN] = {0};
    int size = 6 * 2;
    u8 *all_strbuff = NULL;
    ...
    if(all_strbuff!=NULL)
    {
        ...
        snprintf(buff, sizeof (buff), "%08X", info->edge_palm_rej_enabled);
        strncat(all_strbuff, buff, size);
...
static ssize_t fts_edge_palm_rej_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char *p = (char *)buf;
    unsigned int temp;
    int res;
...
//cat charger_mode		    to show the status of the charger_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
...
//cat glove_mode		    to show the status of the glove_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
...
//cat vr_mode		    to show the status of the vr_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
//echo 01/00 > vr_mode; cat vr_mode 		to enable/disable vr mode and see the switch status in just one call
static ssize_t fts_vr_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char buff[CMD_STR_LEN] = {0};
    int size = 6 * 2;
    u8 *all_strbuff = NULL;
...
static ssize_t fts_vr_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char *p = (char *)buf;
    unsigned int temp;
    int res;
...
//cat stylus_mode		    to show the status of the stylus_enabled switch (example output in the terminal = "AA00000001BB" if the switch is enabled)
...
//echo EE X1 X2 ... X8 > gesture_mask     set the gesture mask to disable/enable; EE = 00(disable) or 01(enable); X1 ... X8 = gesture mask (example 06 00 ... 00 this gesture mask represent the gestures with ID = 1 and 2) can be specified from 1 to 8 bytes, if less than 8 bytes are specified the remaining bytes are kept as previous settings
//cat gesture_mask		    enable/disable the given mask, if one or more gestures is enabled the driver will automatically enable the gesture mode. If all the gestures are disabled the driver automatically will disable the gesture mode. At the end an error code will be printed (example output in the terminal = "AA00000000BB" if there are no errors)
//echo EE X1 X2 ... X8 > gesture_mask; cat gesture_mask 		perform in one command both actions stated before
...
#ifdef USE_CUSTOM_GESTURES			//allow to use user defined gestures
//echo ID X1 Y1 X2 Y2 ... X30 Y30 > add_custom_gesture     add a custom gesture; ID = 1 byte that represent the gesture ID of the custom gesture (can be chosen only between the custom IDs defined in ftsGesture.h); X1 Y1 ... = a series of 30 points (x,y) which represent the gesture template. The loaded gesture is enabled automatically
//cat add_custom_gesture/remove_custom_gesture		    Print the error code of the last operation performed with the custom gestures (example output in the terminal = "AA00000000BB" if there are no errors)
//echo ID X1 Y1 X2 Y2 ... X30 Y30 > add_custom_gesture; cat add_custom_gesture 		perform in one command both actions stated before
static ssize_t fts_custom_gesture_result_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char buff[CMD_STR_LEN] = {0};
    int size = 6 * 2;
    u8 *all_strbuff = NULL;
    ...
    logError(0, "%s fts_custom_gesture_result_show: Last Operation Result = %08X \n", tag, custom_gesture_res);
    ...
    {
        logError(1, "%s fts_custom_gesture_result_show: Unable to allocate all_strbuff! ERROR %08X\n", tag, ERROR_ALLOC);
    }
...
static ssize_t fts_add_custom_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char *p = (char *)buf;
    int n;
    unsigned int temp;
    ...
    {
        logError(1, "%s fts_add_custom_gesture_store: Number of bytes of parameter wrong! %d != %d \n", tag, (count + 1) / 3,GESTURE_CUSTOM_POINTS+1);
        res = ERROR_OP_NOT_ALLOW;
    }
    else
    {
        ...
        if(res>=OK)
        {
            logError(1, "%s fts_add_custom_gesture_store: Adding custom gesture ID = %02X... \n", tag, gestureID);
            res = addCustomGesture(template, GESTURE_CUSTOM_POINTS, gestureID);
            if (res < OK)
            {
                logError(1, "%s fts_add_custom_gesture_store: error during add of custom gesture... ERROR %08X \n", tag, res);
            }
            else
            {
                logError(1, "%s fts_add_custom_gesture_store: Enabling in the gesture mask... \n", tag);
                gestureIDtoGestureMask(gestureID, gestMask);
                res = enableGesture(gestMask, GESTURE_MASK_SIZE);
                if (res < OK)
                {
                    logError(1, "%s fts_add_custom_gesture_store: error during enable gesture mask... ERROR %08X \n", tag, res);
                }
                else
                {
                    //if(check_feature_feasibility(info,FEAT_GESTURE)==OK)
                    //info->gesture_enabled = isAnyGestureActive();			//uncomment if you want to activate automatically the gesture mode when a custom gesture is loaded
                    logError(1, "%s fts_add_custom_gesture_store: Custom Gesture enabled! \n", tag, res);
                }
...
//echo ID > remove_custom_gesture     remove a custom gesture; ID = 1 byte that represent the gesture ID of the custom gesture (can be chosen only between the custom IDs defined in ftsGesture.h); the same gesture is disabled automatically
static ssize_t fts_remove_custom_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    char *p = (char *)buf;
    unsigned int temp;
    int res;
    ...
    if ((count + 1) / 3 < 1)
    {
        logError(1, "%s fts_remove_custom_gesture_store: Number of bytes of parameter wrong! %d != %d \n", tag, (count + 1) / 3,1);
        res = ERROR_OP_NOT_ALLOW;
    }
    else
    {
        ...
        if(res>=OK)
        {
            logError(1, "%s fts_remove_custom_gesture_store: Removing custom gesture ID = %02X... \n", tag, gestureID);
            res = removeCustomGesture(gestureID);
            if (res < OK)
            {
                logError(1, "%s fts_remove_custom_gesture_store: error during add of custom gesture... ERROR %08X \n", tag, res);
            }
            else
            {
                logError(1, "%s fts_remove_custom_gesture_store: Enabling in the gesture mask... \n", tag);
                gestureIDtoGestureMask(gestureID, gestMask);
                res = disableGesture(gestMask, GESTURE_MASK_SIZE);
                if (res < OK)
                {
                    logError(1, "%s fts_remove_custom_gesture_store: error during enable gesture mask... ERROR %08X \n", tag, res);
                }
                else
                {
                    //if(check_feature_feasibility(info,FEAT_GESTURE)==OK)
                    //info->gesture_enabled = isAnyGestureActive();			//uncomment if you want to disable automatically the gesture mode when a custom gesture is removed and no other gestures were enabled
                    logError(1, "%s fts_remove_custom_gesture_store: Custom Gesture disabled! \n", tag, res);
                }
...
// X1X2 Y1Y2 ... = X1X2 2 bytes in HEX format for x[i] and Y1Y2 2 bytes in HEX format for y[i] (MSB first)
//*********************************
// BB = end byte
static ssize_t fts_gesture_coordinates_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if(all_strbuff!=NULL)
    {
        ...
        if(res>=OK)
        {
            snprintf(buff, sizeof (buff), "%02X", gesture_coords_reported);
            strncat(all_strbuff, buff, size);

            for(i=0; i<gesture_coords_reported; i++)
            {
                snprintf(buff, sizeof (buff), "%04X", gesture_coordinates_x[i]);
                strncat(all_strbuff, buff, size);
                snprintf(buff, sizeof (buff), "%04X", gesture_coordinates_y[i]);
                strncat(all_strbuff, buff, size);
            }
...
#ifdef EDGE_REJ
static DEVICE_ATTR(edge_rej, (S_IRUGO | S_IWUSR | S_IWGRP), fts_edge_rej_show, fts_edge_rej_store);
#endif

#ifdef CORNER_REJ
static DEVICE_ATTR(corner_rej, (S_IRUGO | S_IWUSR | S_IWGRP), fts_corner_rej_show, fts_corner_rej_store);
#endif

#ifdef EDGE_PALM_REJ
static DEVICE_ATTR(edge_palm_rej, (S_IRUGO | S_IWUSR | S_IWGRP), fts_edge_palm_rej_show, fts_edge_palm_rej_store);
#endif
...
#ifdef VR_MODE
static DEVICE_ATTR(vr_mode, (S_IRUGO | S_IWUSR | S_IWGRP), fts_vr_mode_show, fts_vr_mode_store);
#endif
...
#ifdef USE_CUSTOM_GESTURES
static DEVICE_ATTR(add_custom_gesture, (S_IRUGO | S_IWUSR | S_IWGRP), fts_custom_gesture_result_show, fts_add_custom_gesture_store);
static DEVICE_ATTR(remove_custom_gesture, (S_IRUGO | S_IWUSR | S_IWGRP), fts_custom_gesture_result_show, fts_remove_custom_gesture_store);
#endif
#endif
...
#ifdef STYLUS_MODE
//TODO: check with ST how FW report a stylus touch in the touch event, this is an example code
    ...
    if(*event == 0x03 && touchId == 0 && g_iTouchDownCount == 0)
		logError(0, "%s: Trace touch down. Event 0x%02x-ID[%d]=%3d,%3d,%3d\n", tag, *event, touchId, x, y, z);

	if(g_iTouchDownCount++ >= 1)
		g_iTouchDownCount = 0;
...
//TODO: check with ST how FW report a stylus touch in the touch event, this is an example code
    ...
    //if(touchId == 0)
		//logError(0, "%s: Trace touch release. Event 0x%02x-ID[%d]\n", tag, event[0], touchId);
    //return fts_next_event(event);
}
...
#ifdef PHONE_GESTURE
//TODO: Customer should implement their own action in respons of a gesture event. This is an example that simply print the gesture received
...
static int cx_crc_check(void)
{
    ...
    if (res < OK)
    {
        logError(1, "%s %s Cannot send system resest command ERROR %08X\n", tag, __func__, res);
        return res;
    }
    setSystemResettedDown(1);
    setSystemResettedUp(1);
    ...
    if ( res < OK)
    {
        logError(1,"%s cx_crc_check: No Config CRC Found! \n",tag);
    }
    else
    {
        if(readData[2] == CRC_CONFIG_SIGNATURE || readData[2] == CRC_CONFIG)
        {
            logError(1,"%s cx_crc_check: CRC Error for config found! CRC ERROR = %02X\n",tag, readData[2]);
            return readData[2];
        }
    }
...
//SW8-DH Disable for bring up, will be enabled later after verifing by real panel-[
    // check CRC status
    ret = cx_crc_check();
...
//SW8-DH Disable for bring up, will be enabled later after verifing by real panel-]
    ...
    if (ret < OK)
    {
        logError(1, "%s %s Cannot send system reset command ERROR %08X\n", tag, __func__, ret);
    }
    else
    {
        setSystemResettedDown(1);
        ...
        else
        {
            ...
            {
                logError(1,"%s %s: CRC Error for CX found! CRC ERROR = %02X\n\n",tag, __func__, readData[2]);
                flag_init=1;
            }
        }
    }
    ...
    if (((ftsInfo.u32_mpPassFlag != INIT_MP) && (ftsInfo.u32_mpPassFlag != INIT_FIELD)) || (ftsInfo.u8_msScrConfigTuneVer!=ftsInfo.u8_msScrCxmemTuneVer || ftsInfo.u8_ssTchConfigTuneVer!=ftsInfo.u8_ssTchCxmemTuneVer)
#ifndef FTM3_CHIP
            || flag_init ==1
#endif
	...
	else
	{
		...
		if(g_usb_status == 1)
		{
			logError(0, "%s %s: Do touch enter charger mode after update fw\n", tag, __func__);
			fts_touch_notify_charger_connect(g_usb_status);
		}
	}
    logError(1, "%s Fw Auto Update Finished! Fiwmware version is ST-%04X.%04X\n", tag, ftsInfo.u16_fwVer, ftsInfo.u16_cfgId); //SW-PRODUCTION-JH-Porting touch*
}
...
static int fts_chip_initialization(struct fts_ts_info *info)
{
    ...
    {
        ret2 = production_test_main(LIMITS_FILE, 1, 1, &todoDefault, INIT_FIELD);
        if (ret2 == OK)
            break;
        initretrycnt++;
...
int fts_chip_powercycle2(struct fts_ts_info *info,unsigned long sleep)
{
    ...
    mdelay(500); //time needed by the regulators for reaching the regime values
...
static int fts_mode_handler(struct fts_ts_info *info,int force)
{
    ...
    case 1:	//screen up
...
#endif
#ifdef FEAT_STYLUS
        if((info->stylus_enabled == FEAT_ENABLE && isSystemResettedUp()) || force==1)
        {
            logError(0, "%s %s: Stylus Mode setting... \n", tag, __func__);
            ret=featureEnableDisable(info->stylus_enabled,FEAT_STYLUS);
            if(ret<OK)
            {
                logError(1, "%s %s: error during setting STYLUS_MODE! ERROR %08X\n", tag, __func__,ret);
            }
            res |=ret;
...
#endif
#ifdef FEAT_VR
        if((info->vr_enabled == FEAT_ENABLE && isSystemResettedUp()) || force==1)
        {
            logError(0, "%s %s: Vr Mode setting... \n", tag, __func__);
            ret=featureEnableDisable(info->vr_enabled,FEAT_VR);
            if(ret<OK)
            {
                logError(1, "%s %s: error during setting VR_MODE! ERROR %08X\n", tag, __func__,ret);
            }
            res |=ret;
        ...
        if((info->edge_rej_enabled == FEAT_ENABLE && isSystemResettedUp()) || force==1)
        {
            ...
            if(ret<OK)
            {
                logError(1, "%s %s: error during setting EDGE_REJECTION_MODE! ERROR %08X\n", tag, __func__,ret);
            }
            res |=ret;
            ...
            if(ret>=OK && info->edge_rej_enabled == FEAT_ENABLE)
            {
                ...
                logError(1, "%s %s: EDGE_REJECTION_MODE Enabled! \n", tag, __func__);
            }
            else
            {
                logError(1, "%s %s: EDGE_REJECTION_MODE Disabled! \n", tag, __func__);
            }
        ...
        if((info->corner_rej_enabled == FEAT_ENABLE && isSystemResettedUp()) || force==1)
        {
            ...
            if(ret<OK)
            {
                logError(1, "%s %s: error during setting CORNER_REJECTION_MODE! ERROR %08X\n", tag, __func__,ret);
            }
            res |=ret;
            ...
            if(ret>=OK && info->corner_rej_enabled == FEAT_ENABLE)
            {
                ...
                logError(1, "%s %s: CORNER_REJECTION_MODE Enabled! \n", tag, __func__);
            }
            else
            {
                logError(1, "%s %s: CORNER_REJECTION_MODE Disabled! \n", tag, __func__);
            }
        ...
        if((info->edge_palm_rej_enabled == FEAT_ENABLE && isSystemResettedUp()) || force==1)
        {
            ...
            if(ret<OK)
            {
                logError(1, "%s %s: error during setting EDGE_PALM_REJECTION_MODE! ERROR %08X\n", tag, __func__,ret);
            }
            res |=ret;
            ...
            if(ret>=OK && info->edge_palm_rej_enabled == FEAT_ENABLE)
            {
                ...
                logError(1, "%s %s: EDGE_PALM_REJECTION_MODE Enabled! \n", tag, __func__);
            }
            else
            {
                logError(1, "%s %s: EDGE_PALM_REJECTION_MODE Disabled! \n", tag, __func__);
            }
                          ...
                          bool enable)
{
    ...
    if (info->bus_reg)
    {
        ...
        logError(1, "[FIH] %s %s: enable bus regulator\n", tag, __func__);
        if (retval < 0)
        {
    ...
    if (info->pwr_reg)
    {
        ...
        logError(1, "[FIH] %s %s: enable pwr regulator\n", tag, __func__);
        if (retval < 0)
        {
...
static int fts_set_gpio(struct fts_ts_info *info)
{
    ...
    //logError(1, "%s %s: [FIH] Config reset pin to 1 \n", tag, __func__);
    //retval = fts_gpio_setup(89, true, 1, 1);
...
void touch_tpfwver_read(char *fw_ver)
{
    snprintf(fw_ver, 128, "ST-%04X.%04X\n", ftsInfo.u16_fwVer, ftsInfo.u16_cfgId);
}
void touch_vendor_read(char *vendor)
{
    snprintf(vendor, 128, "%s\n", "ST");
}

void stm_fts_fih_cmd(void)
{
    char buff[CMD_STR_LEN] = {0};
    int res = OK, i = 0, j = 0, doClean = 0, size = 6 * 2, mp_test_result = 1;
	bool bIsMpTest = false;
    int icommand = 0;
    u8 *all_strbuff = NULL;
	...
	4:	MS CX DATA TEST
	5:	SS RAW DATA TEST
	6:	SS IX CX DATA TEST
	13:	MS Raw Frame
	14:	MS Init Data
	15:	SS Raw Frame
	16:	SS Init Data
	*********************/
    u32 fih_typeOfComand[1] = {0x00};
	...
	{
		icommand = sizeof(fih_typeOfComand)/sizeof(u32);
        logError(0, "%s stm_fts_fih_cmd: icommand = %d \n", tag, icommand);
	}
    for(i = 0; i < icommand; i++)
    {
        logError(0, "%s========================================================================\n",tag);
        logError(0, "%s stm_fts_fih_cmd: Start execute test case: (%d) 0x%2X \n", tag, i, fih_typeOfComand[i]);

        memset(&compData,    0x0, sizeof(MutualSenseData));
        memset(&comData,     0x0, sizeof(SelfSenseData));
        memset(&frameMS,     0x0, sizeof(MutualSenseFrame));
        memset(&frameSS,     0x0, sizeof(SelfSenseFrame));
        memset(&todoDefault, 0x0, sizeof(TestToDo));
        bIsMpTest = false;
		mp_test_result = 1;
		stSelfTestResult = 1;
        ...
        switch (fih_typeOfComand[i])
        {
        /*ITO TEST*/
        case 0x01:
            logError(0, "%s [ ITO TEST ], res = %08X \n", tag, res);
            res = production_test_ito();
            break;
        /*PRODUCTION TEST*/
        case 0x00:
            logError(0, "%s [ PRODUCTION TEST ], res = %08X \n", tag, res);
            if (ftsInfo.u32_mpPassFlag!=INIT_MP)
            {
                logError(0, "%s MP Flag not set! \n", tag, res);
                res = production_test_main(LIMITS_FILE, 1, 1, &todoDefault, INIT_MP);
        ...
        /*read mutual raw*/
        case 0x13:
            logError(0, "%s [ MS Raw Frame ], res = %08X \n", tag, res);
            logError(0, "%s Get 1 MS Frame \n", tag);
            //res = getMSFrame(ADDR_RAW_TOUCH, &frame, 0);
            res = getMSFrame2(MS_TOUCH_ACTIVE, &frameMS);
            if (res < 0)
            ...
            else
            {
                logError(0, "%s The frame size is %d words\n", tag, res);
                size = (res * sizeof (short) + 8)*2;
                /* set res to OK because if getMSFrame is successful res = number of words read*/
                res = OK;
                print_frame_short("MS frame =", array1dTo2d_short(frameMS.node_data, frameMS.node_data_size, frameMS.header.sense_node), frameMS.header.force_node, frameMS.header.sense_node);
            }
            break;
        /*read self raw*/
        case 0x15:
            logError(0, "%s [ SS Raw Frame ], res = %08X \n", tag, res);
            logError(0, "%s Get 1 SS Frame \n", tag);
            res = getSSFrame2(SS_TOUCH, &frameSS);
        ...
        case 0x14: //read mutual comp data
            logError(0, "%s [ MS Init Data ], res = %08X \n", tag, res);
            logError(0, "%s Get MS Compensation Data \n", tag);
            res = readMutualSenseCompensationData(MS_TOUCH_ACTIVE, &compData);
        ...
        case 0x16: //read self comp data
            logError(0, "%s [ SS Init Data ], res = %08X \n", tag, res);
            logError(0, "%s Get SS Compensation Data... \n", tag);
            res = readSelfSenseCompensationData(SS_TOUCH, &comData);
            if (res < 0)
            {
        ...
        case 0x03: // MS Raw DATA TEST
            logError(0, "%s [ MS Raw Frame Test ], res = %08X \n", tag, res);
            res = fts_system_reset();
            if (res >= OK)
                res = production_test_ms_raw(LIMITS_FILE, 1, &todoDefault);
            break;
        ...
        case 0x04: // MS CX DATA TEST
            logError(0, "%s [ MS CX DATA TEST ], res = %08X \n", tag, res);
            res = fts_system_reset();
            if (res >= OK)
                res = production_test_ms_cx(LIMITS_FILE, 1, &todoDefault);
            break;
        ...
        case 0x05: // SS RAW DATA TEST
            logError(0, "%s [ SS RAW DATA TEST ], res = %08X \n", tag, res);
            res = fts_system_reset();
            if (res >= OK)
                res = production_test_ss_raw(LIMITS_FILE, 1, &todoDefault);
            break;
        ...
        case 0x06: // SS IX CX DATA TEST
            logError(0, "%s [ SS IX CX DATA TEST ], res = %08X \n", tag, res);
            res = fts_system_reset();
            if (res >= OK)
                res = production_test_ss_ix_cx(LIMITS_FILE, 1, &todoDefault);
            break;
        ...
        case 0xF0:
        case 0xF1: // TOUCH ENABLE/DISABLE
            doClean = (int) (fih_typeOfComand[i] & 0x01);
            logError(0, "%s  [ TOUCH ENABLE/DISABLE ] = %d\n", tag, doClean);
            res = cleanUp(doClean);
            break;
...
END: //here start the reporting phase, assembling the data to send in the file node
        ...
        if (res >= OK)
        {
            ...
            case 0x15:
                ...
                if(frameSS.force_data) kfree(frameSS.force_data);
                if(frameSS.sense_data) kfree(frameSS.sense_data);
                break;
            ...
            case 0x14:
                ...
                if(compData.node_data) kfree(compData.node_data);
                break;
        ...
        //Judge pass or fail
        //SUCCESS="0"FAILED="1", for FTM script
        if(bIsMpTest)
        {
            if(mp_test_result == OK)
            {
                stSelfTestResult = 0;
                logError(1, "%s The test result is ****PASS**** %s, res = 0x%08X\n", tag, all_strbuff, res);
            }
            else
            {
                logError(1, "%s The test result is ****FAIL**** %s, res = 0x%08X\n", tag, all_strbuff, res);
            }
        }
        //logError(0,"%s numberParameters = %d \n",tag, numberParameters);
        if(all_strbuff)
	...
	//Final action
	/*
            After executing Selftest in FQC, touch need to suspend and resume to enable touch
	*/
#if 0
    if(g_info != NULL)
    {
	    //Suspend
	    logError(0, "%s %s: Suspend after self test\n", tag, __func__);
	    queue_work(g_info->event_wq, &g_info->suspend_work);

		msleep(50);

	    //Resume
	    logError(0, "%s %s: Resume after self test\n", tag, __func__);
	    queue_work(g_info->event_wq, &g_info->resume_work);
    }
	else
	{
		logError(0, "%s %s: ERROR: g_info is NULL after self test\n", tag, __func__);
	}
#else
	res = cleanUp(1);//Enable touch
    if(res<OK)
    {
        logError(1,"%s %s: could not start scanning! ERROR %08X. CAN ignore this error \n",tag,__func__,res);
        //goto END; //Can ignore it
    }
    mdelay(WAIT_FOR_FRESH_FRAMES);
#endif
}
...
int stm_fts_fih_read_result(void)
{
    logError(1, "%s stm_fts_fih_read_result::stSelfTestResult = %d\n", tag, stSelfTestResult);
    return stSelfTestResult;
}
unsigned int touch_double_tap_read(void)
{
	logError(1, "%s, touch_double_tap_read double_tap = %d\n", tag, double_tap_enable);
...
int touch_double_tap_write(unsigned int enable)
{
	struct fts_ts_info *info = g_info;
    char buff[CMD_STR_LEN] = {0};
    static u8 gesture_mask[GESTURE_MASK_SIZE+2];
    int size = 6 * 2;
    u8 *all_strbuff = NULL;
    int res,temp;

	logError(1,  "%s current double_tap = %d, set double_tap = %d\n", tag, double_tap_enable, enable);
	...
	res = updateGestureMask(&gesture_mask[2], gesture_mask[0], gesture_mask[1]);
	...
	if(all_strbuff!=NULL)
	{
		...
		if(res == OK)
		{
			if(enable)
				...
				double_tap_enable = 0;
			logError(1, "%s touch_double_tap_write ****PASS**** %s, res = 0x%08X\n", tag, all_strbuff, res);
		}
		else
		{
			double_tap_enable = -1;
			logError(1, "%s touch_double_tap_write ****FAIL**** %s, res = 0x%08X\n", tag, all_strbuff, res);
		}
	...
	{
		logError(1, "%s touch_double_tap_write: Unable to allocate all_strbuff! ERROR %08X\n", tag, ERROR_ALLOC);
	}
	gesture_mask[0] = 0;
	return 0;
}
...
void fts_fih_tp_enable(int flag)
{
	   ...
	   if(g_info != NULL)
	   {
	       if(flag == 1)
	       {
	             ...
	             hrtimer_start(&g_info->timer_tpalt, ktime, HRTIMER_MODE_REL);
	       }
	       else if(flag == 0)
	       {
	              pr_info("F@TOUCH %s flag = %d ",__func__,flag);
	              hrtimer_cancel(&g_info->timer_tpalt);
	              cancel_work_sync(&g_info->tpalt_work);
	              flush_workqueue(g_info->alt_test_wq);
	       }
	       else
	       {
	              pr_info("F@TOUCH %s, flag = %d ",__func__, flag);
...
void fts_touch_notify_charger_connect(u32 type)
{
	int res = OK;
	g_usb_status = type;
	logError(1, "%s %s: Got Charging %s notification\n", tag, __func__, (g_usb_status) ? "enable" : "disable");
    if((tp_probe_success == 1) && (g_info != NULL) && (g_auto_update_fw == 1))
    {
		g_info->charger_enabled = g_usb_status;
		res = check_feature_feasibility(g_info, FEAT_CHARGER);
		if(res >= OK)
		{
			res = fts_mode_handler(g_info, 1);
			logError(1, "%s %s: Charger %s %s\n", tag, __func__, (g_info->charger_enabled) ? "Enable" : "Disable",(res >= OK) ? "SUCCESS" : "FAIL");
		}
		else
			logError(1, "%s %s: Charger %s %s Not allow\n", tag, __func__, (g_info->charger_enabled) ? "Enable" : "Disable",(res >= OK) ? "SUCCESS" : "FAIL");
    }
	else
	{
		//For boot device and USB connect case
		if(g_usb_status == 1)
		{
			if(g_auto_update_fw == 0)
				logError(0, "%s %s: Touch driver not do auto update fw yet while probe\n", tag, __func__);
			else
				logError(0, "%s %s: g_info is NULL, do it later after touch probe success\n", tag, __func__);
		}
	}
    return ;
}
...
static enum hrtimer_restart tpalt_timer_handle(struct hrtimer *hrtimer)
{
    ...
    if(g_info != NULL)
    {
	    queue_work(g_info->alt_test_wq, &g_info->tpalt_work);
	    ktime = ktime_set(0,5* NSEC_PER_MSEC);
	    hrtimer_forward_now(&g_info->timer_tpalt,ktime);
    }
	else
	{
		logError(0, "%s %s: F@TOUCH ERROR: g_info is NULL\n", tag, __func__);
...
static void tpalt_input_work_fn(struct work_struct *work)
{
	...
	error = ALT_readChipInfo();			//system reset OK
	if (error < OK)
	{
		tp_st_count++;
		logError(1, "%s Cannot read Chip Info! ERROR %08X\n", tag, error);
                     ...
                     const struct i2c_device_id *idp)
{
    ...
    //info->fwu_workqueue = create_singlethread_workqueue("fts-fwu-queue");			//deprecated
    ...
    //info->event_wq = create_singlethread_workqueue("fts-event-queue");			//deprecated
    ...
    touch_cb.touch_selftest = stm_fts_fih_cmd;
    ...
    g_info->alt_test_wq = create_freezable_workqueue("tp_alt_work");
    hrtimer_init(&g_info->timer_tpalt, CLOCK_BOOTTIME, HRTIMER_MODE_REL);
    g_info->timer_tpalt.function = tpalt_timer_handle;
    INIT_WORK(&g_info->tpalt_work, tpalt_input_work_fn);
    //SW8-JH-ALT test+]
    tp_probe_success = 1;
    //SW8-PRODUCTION-JH-Porting touch+]

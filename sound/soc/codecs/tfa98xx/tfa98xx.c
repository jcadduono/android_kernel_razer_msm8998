...
static spinlock_t cont_load_lock;
							...
							unsigned int mask)
{
	...
	if (reg & errs & ~mask) {
		...
		dev_err(tfa98xx->codec->dev, "tfa98xx: status_reg events: 0x%x\n", reg);
	}
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	/* Not necessary to load container twice for dual amp case */
	if (container == NULL) {
		container = kzalloc(cont->size, GFP_KERNEL);
		if (!container) {
			release_firmware(cont);
			pr_err("tfa98xx: Error allocating memory\n");
			spin_unlock_irqrestore(&cont_load_lock, flags);
			return;
		}
		...
		dev_dbg(&tfa98xx->i2c->dev, "tfa98xx: tfa_load_cnt done\n");
	}
	else {
		dev_dbg(&tfa98xx->i2c->dev, "tfa98xx: container has been loaded\n");
	}
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	if(fih_hwid_fetch(FIH_HWID_REV)<=FIH_REV_EVT1)
	{
		return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
	                               fw_nameEVT, tfa98xx->dev, GFP_KERNEL,
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		 ...
		 */
		val = snd_soc_read(tfa98xx->codec, TFA98XX_STATUSREG);
		pr_err("tfa98xx: SYS_STATUS0: 0x%04x\n", val);
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	dev_err(codec->dev, "tfa98xx: tfa98xx codec registered (%s)",
							tfa98xx->fw.name);

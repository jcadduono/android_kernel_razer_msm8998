...
#define USE_QC_HIFI_AMP
#define USE_QC_SPK_AMP
#define USE_QC_MBHC
#else
...
#if defined(CONFIG_FIH_9800) || defined(CONFIG_FIH_9802) || defined(CONFIG_FIH_RCL)
...
static struct snd_soc_dai_link msm_mi2s_be_dai_links[] = {
	...
	},
	{
		...
		.platform_name = "msm-pcm-routing",
#if defined(CONFIG_FIH_9801) || defined(CONFIG_FIH_9802) || defined(CONFIG_FIH_9800) || defined(CONFIG_FIH_RCL)
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(pdata->hac_amp_en_gpio) && (!pdata->hac_amp_en_gpio_p)) {
		dev_info(&pdev->dev, "property %s not detected in node %s",
			"fih,hac-amp-en-gpio", pdev->dev.of_node->full_name);
	} else {
		dev_dbg(&pdev->dev, "%s detected",
			"fih,hac-amp-en-gpio");
	}
